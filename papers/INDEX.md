...
| Upstream hot-paths regression | Commit `a5c87f9` changes search behavior, incompatible with PCG (8.7× regression). Pre-selected seeds biased: MP's 2.35× speedup reverses to 2.3× slowdown on random seeds (Exp 5). Issue [#4](https://github.com/PKU-CMEGroup/MCTS-4-SR/issues/4). **Superseded:** with upstream's new HP (c=6, gp_rate=0.5), these effects are negligible — seed quality and MP have no measurable impact. | [huang2025_hotpaths_regression.md](./huang2025_hotpaths_regression.md) |
...
| SRBench seed provenance | The 100 benchmark seeds are `RandomState(42).randint(0,32767,100)` from SRBench (2020). All SR methods use MT19937. 100-seed benchmarks show seed quality has **no effect** with upstream's upgraded HP (c=6, gp_rate=0.5): <0.3% difference across MT+SRBench, MT+quantum, PCG+quantum. Seed bias only mattered under paper HP (c=1, gp_rate=0.2), which was more exploration-heavy and thus more sensitive to rollout randomness. | [srbench_seed_provenance.md](./srbench_seed_provenance.md) |
...
1. ~~**`huang_notes.md`**~~ → Done: `huang2025_notes.md`. Covers architecture, evaluation pipeline, state-jumping mechanics, weaknesses (blind expansion, max-only selection, ghost mutations), and relevance to our work.
