...
from ..transformer.mobius_transform import mobius_transform, select_dominant_coefficients
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        while True:
            try:
                ...
                # Compute Möbius coefficients from full truth table
                function_values = evaluations[:, -1]  # Extract function values (last column)
                mu = mobius_transform(function_values)
                coeff_input = select_dominant_coefficients(mu, evaluations)
                ...
                if tokenized_evaluations is not None and tokenized_expression is not None:
                    return tokenized_evaluations, tokenized_expression, coeff_input
